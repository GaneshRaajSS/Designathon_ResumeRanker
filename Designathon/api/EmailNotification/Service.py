from db.Model import EmailNotification, User, WorkflowStatus, JobDescription, Ranking,ConsultantProfile, SimilarityScore
...
def send_email_with_consultant_report(jd_id: str, recipient_email: str) -> dict:
    ...
    try:
        ...
        # Fetch top 3 ranked consultants
        rankings = (
            db.query(Ranking)
        ...
        for rank in rankings:
            ...
            if profile:
                similarity = (
                    ...
                    .filter_by(jd_id=jd_id, profile_id=profile.id)
                    .order_by(SimilarityScore.created_at.desc())
                    .first()
                )
                consultants.append({
                    ...
                    "name": profile.name,
                    "email": profile.email,
                    "score": similarity.similarity_score if similarity else 0.0,
                    "explanation": rank.explanation
        })

        # Generate and fetch the consultant report
        ...
        # Generate SAS resume links
        sas_resume_links = {
            c["consultant_id"]: generate_sas_url(f"JD/{jd_id}/{c['consultant_id']}.pdf")
            for c in consultants
        }

        # Compose the styled HTML body
        html_body = f"""
        <html>
          <body style="font-family: Arial, sans-serif; color: #333;">
            <h2 style="color: #fa541c;">Your Consultant Ranking Report is Ready</h2>
        ...
        message = {
            ...
            "content": {
                "subject": f"Your JD {jd_id} has been ranked successfully",
                "plainText": "Greetings, please find the attached consultant ranking report.",
                "html": html_body
            },
            "recipients": {
        ...
        client = EmailClient.from_connection_string(os.getenv("AZURE_COMM_EMAIL_CONNECTION_STRING"))
